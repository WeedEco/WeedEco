wplot_geog<-function(model, x, xlims= NULL,ticks =NULL, col1="black",col2= "black",col3="black", pch1=1,pch3=5, compact= F, priority= "descending", site= "Samples", lcol=col3, lpch=pch3,lines=T, legend=T){
 ...
 if(model=='model2'|model== 2) {
  data.model<-data.frame(model2)
    ...
    if(legend==T){legend(max+0.5,0.14,  c( "Evvia fields", "Evvia gardens"), pch=c(2,17), col=col2, cex=0.85, bty="n")
      legend(max+0.5,0.12,  c( "Haute \nProvence", "Asturias" ), pch=c(0,15), col=col2, cex=0.85, bty="n")
