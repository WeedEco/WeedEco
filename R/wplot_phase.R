...
wplot_phase<-function(model, x, site= c("phase 1", "phase 2", "phase 3", "phase 4", "phase 5"), g1=NULL, g2= NULL, g3=NULL, g4=NULL, g5=NULL, gcol=c("black","black","black","black", "black"),
  ...
  functionalAt$pch[functionalAt$pch=="1"]<-17
  functionalAt$pch[functionalAt$pch=="2"]<-2
  ...
  if(is.null(g3)){
    ...
    points(beeswarm::swarmy(functionalAt$LD1*-1, rep(0.1, 2), side=1, compact = compact, priority = priority),col="black", pch=as.numeric(functionalAt$pch),cex=1.5)
    axis(1,ticks)
    ...
    par(mar = c(0,0,0,0))
    plot(2:3, type='n', xlim = xlim, ylim=c(0,1),axes=F, xlab = "", ylab="")
    points(beeswarm::swarmy(functionalAt$LD1*-1, rep(0.2, 2), side=1, compact = compact, priority = priority),col="black", pch=as.numeric(functionalAt$pch),cex=1.5)
    axis(1,ticks)
    mtext(side =1, text = xlab, line=2, cex=0.8)
    segments(min(centroids$centroid1*-1), 0.58,min(centroids$centroid1*-1),-0.6)
    segments(max(centroids$centroid1*-1), 0.589,max(centroids$centroid1*-1),-0.04 )
    ...
    par(mar = c(0,0,0,0))
    plot(2:3, type='n', xlim = xlim, ylim=c(0,1),axes=F, xlab = "", ylab="")
    points(beeswarm::swarmy(functionalAt$LD1*-1, rep(0.1, 2), side=1, compact = compact, priority = priority),col="black", pch=as.numeric(functionalAt$pch),cex=1.2)
    axis(1,ticks)
    mtext(side =1, text = xlab, line=2, cex=0.8)
    segments(min(centroids$centroid1*-1), 0.58,min(centroids$centroid1*-1),-0.6)
    segments(max(centroids$centroid1*-1), 0.589,max(centroids$centroid1*-1),-0.04 )
